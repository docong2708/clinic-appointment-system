...
public class UserServiceLoggingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        log.info("UserService request started correlationId={} method={} uri={} userId={} email={} roles={}",
                valueOrDash(correlationId), method, uri, valueOrDash(userId), valueOrDash(email), valueOrDash(roles));

        try {
            filterChain.doFilter(request, response);
            long durationMs = System.currentTimeMillis() - startedAt;
            log.info("UserService request completed correlationId={} method={} uri={} status={} durationMs={}",
                    valueOrDash(correlationId), method, uri, response.getStatus(), durationMs);
        } catch (Exception exception) {
            long durationMs = System.currentTimeMillis() - startedAt;
            log.error("UserService request failed correlationId={} method={} uri={} status={} durationMs={} error={}",
