...
export default function PatientPortal() {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-slate-950 min-h-[750px] shadow-2xl rounded-3xl overflow-hidden border border-slate-800 flex flex-col">
                  ...
                  {filteredDoctors.map(doc => (
                    ...
                    >
                      <img 
                        src={doc.image} 
                        alt={doc.name} 
                        className="w-16 h-16 rounded-lg object-cover border border-slate-855"
                      />
              ...
              {mockHistory.map(record => (
                <div key={record.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
                  ...
                  <div className="space-y-2 text-[11px]">
                    <div>
                      <span className="text-slate-500 block text-[9px] uppercase font-bold">Diagnosis</span>
                      <p className="text-slate-200 font-semibold">{record.diagnosis}</p>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[9px] uppercase font-bold">Prescribed Medication</span>
                      <p className="text-amber-400 font-mono font-medium">{record.prescription}</p>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[9px] uppercase font-bold">Clinical Consultation Note</span>
      ...
      <div className="bg-slate-900 border-t border-slate-800 px-6 py-3.5 flex justify-between items-center z-10 sticky bottom-0">
        <button 
